...
- Face-to-face encounters now only start for vanilla-driven NPCs, wait for the last encounter bubble to finish before release, keep the last bubble visible through its natural timer, and add detailed `[HANDOFF]`, `[REBIND]`, and `[MONITOR]` diagnostics around vanilla schedule resume; release now drives vanilla `checkSchedule(...)` directly, keeps NPCs pending until vanilla produces real movement/controller state, retries on later schedule times instead of treating a schedule reload alone as success, uses direct same-map active-slot fallback when possible, and handles cross-map active slots as topology-guided same-map legs with explicit transition warps, door-arrival resolution, and stale-leg retry/clear behavior instead of one remote-map controller. After an NPC reaches the active-slot destination, the mod now settles them safely there instead of replaying route-bearing arrival handoff logic.
