...
- Face-to-face encounters now only start for vanilla-driven NPCs, wait for the last encounter bubble to finish before release, keep the last bubble visible through its natural timer, and add detailed `[HANDOFF]`, `[REBIND]`, and `[MONITOR]` diagnostics around vanilla schedule resume; release now drives vanilla `checkSchedule(...)` directly, keeps NPCs pending until vanilla produces real movement/controller state, retries on later schedule times instead of treating a schedule reload alone as success, uses direct same-map active-slot fallback when possible, and handles cross-map active slots as topology-guided same-map legs with explicit transition warps, door-arrival resolution, and stale-leg retry/clear behavior instead of one remote-map controller. After an NPC reaches the active-slot destination, the mod now settles them safely there instead of replaying route-bearing arrival handoff logic, and the resume worker now validates live movement/controller state against the active schedule slot so wrong-map remote endpoints are rejected and rerouted through the current-slot fallback instead of being accepted as success.
- NPC speech bubbles now keep full sentences intact while using vanilla single-line `showTextAboveHead` rendering, and overlong single sentences split into two balanced continuation bubbles with `...` at the end of the first bubble; the default bubble limit is `50`, the GMCM range remains `40-120`, the new GMCM speech-bubble duration slider defaults to `175%` and scales how long bubbles stay visible for reading, and longer encounter bubbles remain visible for up to `3000ms`.
- Face-to-face encounter chance now defaults to `50%`, and the GMCM dropdown now includes a `25%` option.
