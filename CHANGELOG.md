...
- Face-to-face encounters now only start for vanilla-driven NPCs, pin them temporarily, clear lingering vanilla speech/halt blockers (`textAboveHeadTimer`, `textAboveHeadAlpha`, `movementPause`, and halted/charge state) on release, and then reset vanilla schedule gating state (`lastAttemptedSchedule`, `previousEndPoint`, and `currentScheduleDelay`) so the next vanilla update reprocesses the NPC's schedule naturally from its actual current tile.
