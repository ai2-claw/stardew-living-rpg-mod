...
- Face-to-face encounters now only start for vanilla-driven NPCs, wait for the last encounter bubble to finish before release, keep the last bubble visible through its natural timer, and add detailed `[HANDOFF]`, `[REBIND]`, and `[MONITOR]` diagnostics around vanilla schedule resume; release now drives vanilla `checkSchedule(...)` directly, keeps NPCs pending until vanilla produces real movement/controller state, retries on later schedule times instead of treating a schedule reload alone as success, uses direct same-map active-slot fallback when possible, handles cross-map active slots as topology-guided same-map legs with explicit transition warps, door-arrival resolution, and stale-leg retry/clear behavior instead of one remote-map controller, and now rebinds the active slot again on arrival so vanilla-facing/activity states can resume after the NPC reaches the scheduled spot.
