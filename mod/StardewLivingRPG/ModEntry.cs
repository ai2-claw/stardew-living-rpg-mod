...
public sealed class ModEntry : Mod
{
    ...
    private sealed class PendingVanillaEncounterResume
    {
        ...
        public EncounterFallbackMode FallbackMode { get; set; }
    ...
    private NpcResidenceService? _npcResidenceService;
    ...
    private NpcAmbientIndoorChatterService? _npcAmbientIndoorChatterService;
    ...
    private bool NormalizeGmcmConfigValues()
    {
        ...
        var clampedMaxConversationTurns = Math.Clamp(_config.AutonomyMaximumConversationTurns, _config.AutonomyMinimumConversationTurns, 12);
        if (_config.AutonomyMaximumConversationTurns != clampedMaxConversationTurns)
        {
            _config.AutonomyMaximumConversationTurns = clampedMaxConversationTurns;
            changed = true;
        }
    ...
    private void RegisterDeveloperConsoleCommands(IModHelper helper)
    {
        ...
        helper.ConsoleCommands.Add("slrpg_route_check", "Check route between two locations: slrpg_route_check <from> <to>", OnRouteCheckCommand);
        helper.ConsoleCommands.Add("slrpg_autonomy_force_replan", "Force replan for NPC: slrpg_autonomy_force_replan <npc>", OnAutonomyForceReplanCommand);
    ...
    private void OnSaving(object? sender, SavingEventArgs e)
    {
        ...
        {
            _state.Autonomy.RuntimeSummaries.Clear();
            foreach (var kvp in _autonomyRuntimeByNpcId)
            {
                _state.Autonomy.RuntimeSummaries[kvp.Key] = new AutonomyNpcSummary
                {
                    NpcId = kvp.Key,
                    CompletedBlocks = kvp.Value.CompletedBlocksToday,
                    FailedBlocks = kvp.Value.FailedBlocksToday,
                    Encounters = kvp.Value.EncountersToday,
                    LastStatus = kvp.Value.OverrideStatus.ToString()
                };
            }
        }
    ...
    private NPC? TryResolveRosterNpcUnderCursor(GameLocation location)
    {
        ...
        return GetRosterNpcInteractionCandidates(location)
            ...
            {
                Npc = npc,
                HoverBounds = GetNpcCursorHoverBounds(npc)
            })
            .Where(x => x.HoverBounds.Contains(mouseWorld))
            .OrderByDescending(x => x.HoverBounds.Bottom)
    ...
    {
        var collisionBounds = npc.GetBoundingBox();
        if (npc.Sprite is null)
            return collisionBounds;

        var spriteWidthPx = Math.Max(collisionBounds.Width, npc.Sprite.SpriteWidth * Game1.pixelZoom);
        var spriteHeightPx = Math.Max(collisionBounds.Height, npc.Sprite.SpriteHeight * Game1.pixelZoom);

        return new Rectangle(
            collisionBounds.Center.X - (spriteWidthPx / 2),
            collisionBounds.Bottom - spriteHeightPx,
            spriteWidthPx,
            spriteHeightPx);
    }
    ...
    private void TryApplyNpcChatCursorIndicator()
    {
        ...
        var hoveredNpc = TryResolveRosterNpcUnderCursor(Game1.currentLocation);
        if (hoveredNpc is null)
            return;

        if (Vector2.Distance(hoveredNpc.Tile, Game1.player.Tile) > NpcDialogueHookInteractionRadiusTiles)
            return;
    ...
    {
        if (_npcDialogueHookArmed || menu is null)
            return;

        var isShopMenu = menu is ShopMenu;
        var isDialogueMenu = menu is DialogueBox;
        if (!isShopMenu && !isDialogueMenu)
            return;
    ...
    private void RunAutonomyDailyMaintenance()
    {
        ...
        // Restore vanilla schedules before clearing
        _scheduleOverrideService?.RestoreAllSchedules();
        _scheduleOverrideService?.ClearAllBackups();
        _npcSocialEncounterService?.ClearCompleted();
        _faceToFaceService?.CancelAll("day_end");
    ...
    private void TryTriggerAmbientIndoorAutonomyChatter(UpdateTickedEventArgs e)
    {
        if (!_config.EnableAutonomousRoutines
            || !e.IsMultipleOf(30)
            || _npcAmbientIndoorChatterService is null
            || _npcSocialEncounterService is null
    ...
    private void TryTickNpcSpeechBubbles()
    {
        ...
        {
            _npcSpeechBubbleService.CancelAll();
            if (_npcSocialEncounterService is not null)
            {
    ...
    private bool TryRefreshSpatialTarget(NPC npc, AutonomyRuntimeState runtime, AutonomyPlanBlock block, bool forcePlayer2Replan, bool reserveTarget = true)
    {
        ...
        var nearby = npc.currentLocation?.characters?.Where(character => character is not null).Cast<NPC>() ?? Enumerable.Empty<NPC>();
        var snapshot = _npcAutonomyGoalEngine?.BuildSnapshot(_state, npc, nearby) ?? new NpcContextSnapshot
        {
            NpcId = npc.Name ?? string.Empty,
            CurrentLocation = npc.currentLocation?.Name ?? string.Empty,
            HomeLocation = _npcResidenceService?.ResolveHomeLocation(npc.Name ?? string.Empty, Game1.locations, npc.DefaultMap ?? "Town") ?? string.Empty,
            TimeOfDay = Game1.timeOfDay
        };
        ...
        var needsSemanticPlan = forcePlayer2Replan
            ...
            || block.TargetTile == Point.Zero
            || string.Equals(runtime.StuckReason, "blocked_target", StringComparison.OrdinalIgnoreCase)
        ...
        {
            if (!_npcLocationSpotService.IsLongLivedTileAllowed(location, block.TargetTile, block.TargetZoneId, block.TargetSpotRole))
            {
                runtime.StuckReason = "crowded_spot";
                return false;
            }
        ...
        _npcTileReservationService.Release(npc.Name ?? string.Empty);
        if (!_npcLocationSpotService.TryResolveConcreteSpot(
                location,
                npc,
                block,
                suggestion,
                _npcTileReservationService,
                _walkabilityService,
                out var spot,
                out var tile))
        {
        ...
        block.TargetZoneId = spot.ZoneId;
        ...
        out bool plannedByPlayer2)
    {
        ...
        var homeLocation = _npcResidenceService?.ResolveHomeLocation(npc.Name ?? string.Empty, Game1.locations, snapshot.HomeLocation);
    ...
    private void TryAdvanceAutonomyEncounterStates()
    {
        if (_npcSocialEncounterService is null || _npcSpeechBubbleService is null || _npcConversationSimulationService is null)
            return;
        ...
        foreach (var encounter in _npcSocialEncounterService.GetActiveEncounters().ToArray())
        {
            ...
            if (encounter.Phase == EncounterPhase.Staging)
            {
                ...
                if (!(_faceToFaceService?.IsReadyToTalk(encounter.EncounterId) ?? false))
                    continue;
    ...
    private void TryProcessPendingVanillaEncounterResumes(ulong currentTick)
    {
        ...
        foreach (var npcId in _pendingVanillaEncounterResumeByNpcId.Keys.ToArray())
        {
            ...
            {
                Monitor.Log(
                    $"Autonomy: returned {npc.Name} to active schedule target after encounter {pending.EncounterId} ({pending.Phase}, restored={pending.RestoredSchedule}, attempts={pending.Attempts}, active_schedule_time={(pending.ActiveScheduleTime?.ToString(CultureInfo.InvariantCulture) ?? "none")}, next_schedule_time=none, active_target_location={DescribeText(pending.ActiveTargetLocation)}, active_target_tile={DescribeNullablePoint(pending.ActiveTargetTile)}, fallback_used={pending.UsedTemporaryActiveSlotFallback}, resumed=true, method=ActiveSlotFallback(already_at_target), controller={DescribeControllerType(npc)}, isMoving={npc.isMoving()}, TilePoint=({npc.TilePoint.X},{npc.TilePoint.Y}), map={npc.currentLocation?.Name ?? "unknown"}, time={Game1.timeOfDay}).",
                    LogLevel.Debug);
                _pendingVanillaEncounterResumeByNpcId.Remove(npcId);
                continue;
            }
        ...
        out string checkScheduleMethod)
    {
        ...
        if (!HasVanillaResumeState(npc)
            ...
            && TryForcePathToActiveScheduleEntry(npc, pending, _lastUpdateTick, false, out var fallbackController))
        {
        ...
        out object? fallbackController)
    {
        ...
        return sameMap
            ? TryStartSameMapActiveSlotFallback(npc, pending, targetLocation, currentTick, out fallbackController)
            : TryStartCrossMapActiveSlotFallback(npc, pending, currentTick, out fallbackController);
    }

    private bool TryStartSameMapActiveSlotFallback(
        NPC npc,
        PendingVanillaEncounterResume pending,
    ...
    private bool TryStartCrossMapActiveSlotFallback(
        NPC npc,
        PendingVanillaEncounterResume pending,
        ulong currentTick,
        out object? fallbackController)
    {
        ...
        {
            Monitor.Log(
                $"Autonomy: [CrossMapLeg(start)] {npc.Name} could not plan route from {npc.currentLocation.Name} to {pending.ActiveTargetLocation}.",
                LogLevel.Debug);
            return false;
        }
    ...
    {
        if (pending.FallbackMode != EncounterFallbackMode.CrossMapLeg || npc.currentLocation is null)
            return;

        var currentLocationName = npc.currentLocation.Name ?? string.Empty;
        var currentTile = npc.TilePoint;
        var locationChanged = !string.Equals(currentLocationName, pending.FallbackLastObservedLocation, StringComparison.OrdinalIgnoreCase);
        ...
        if (string.Equals(currentLocationName, pending.FallbackLegToLocation, StringComparison.OrdinalIgnoreCase))
        {
            ...
            {
                Monitor.Log(
                    $"Autonomy: [CrossMapLeg(target_map)] {npc.Name} encounter={pending.EncounterId} reached target map {currentLocationName}; switching to active-slot target fallback.",
                    LogLevel.Debug);
            }
        ...
        Monitor.Log(
            $"Autonomy: [CrossMapLeg(stale)] {npc.Name} encounter={pending.EncounterId} leg={pending.FallbackLegFromLocation}->{pending.FallbackLegToLocation} tile=({currentTile.X},{currentTile.Y}) map={currentLocationName} retry_count={pending.FallbackLegRetryCount}.",
            LogLevel.Debug);
    ...
    private static void ResetActiveSlotFallbackState(PendingVanillaEncounterResume pending)
    {
        PrepareActiveSlotFallbackState(pending, preserveLegRetryCount: false);
        pending.UsedTemporaryActiveSlotFallback = false;
        pending.FallbackControllerStartedAtTime = null;
        pending.TemporaryFallbackController = null;
    }
    ...
    private bool TryPlanNextCrossMapFallbackLeg(NPC npc, string targetLocationName, out RouteSegment segment)
    {
        ...
        if (_worldTopologyService is null || npc.currentLocation is null || string.IsNullOrWhiteSpace(targetLocationName))
            return false;

        var currentLocationName = npc.currentLocation.Name;
        if (string.IsNullOrWhiteSpace(currentLocationName))
            return false;

        var graph = _worldTopologyService.BuildGraph(Game1.locations);
        var routePlannerService = _routePlannerService;
        if (routePlannerService is not null)
        {
            var route = routePlannerService.PlanRoute(graph, npc.Name, currentLocationName, targetLocationName, int.MaxValue / 4);
        ...
        var edgePath = _worldTopologyService.FindRoute(graph, currentLocationName, targetLocationName);
    ...
    private bool TryResolveCrossMapLegDepartureTile(NPC npc, Point requestedTile, out Point departureTile)
    {
    ...
    private static bool IsMapTransitionTile(GameLocation location, Point tile)
    {
        if (location.warps is not null)
        {
            ...
            {
                if (warp is not null && warp.X == tile.X && warp.Y == tile.Y)
                    return true;
            }
        }
        ...
        if (location.doors?.Pairs is not null)
        {
            ...
            {
                if (door.Key.X == tile.X && door.Key.Y == tile.Y)
                    return true;
            }
        }
    ...
    private bool IsWithinAutonomyTalkRange(NPC speakerNpc, NPC listenerNpc)
    {
        if (speakerNpc.currentLocation is null || listenerNpc.currentLocation is null)
            return false;
        if (!string.Equals(speakerNpc.currentLocation.Name, listenerNpc.currentLocation.Name, StringComparison.OrdinalIgnoreCase))
            return false;
    ...
    private void OnRouteCheckCommand(string name, string[] args)
    {
        if (args.Length < 2)
        {
            Monitor.Log("Usage: slrpg_route_check <from_location> <to_location>", LogLevel.Info);
            return;
        }

        if (_worldTopologyService is null)
        {
            Monitor.Log("WorldTopologyService not available.", LogLevel.Warn);
            return;
        }

        var graph = _worldTopologyService.BuildGraph(Game1.locations);
        var route = _worldTopologyService.FindRoute(graph, args[0], args[1]);
        if (route is null || route.Count == 0)
        {
            Monitor.Log($"No route found from '{args[0]}' to '{args[1]}'.", LogLevel.Info);
            return;
        }

        var totalMinutes = _worldTopologyService.EstimateTotalMinutes(route);
        Monitor.Log($"Route: {args[0]} → {args[1]} ({route.Count} hops, ~{totalMinutes} min)", LogLevel.Info);
        for (var i = 0; i < route.Count; i++)
        {
            var edge = route[i];
            Monitor.Log($"  [{i + 1}] {edge.FromLocationId} → {edge.ToLocationId} (~{edge.EstimatedTravelMinutes} min, warp={edge.IsWarp}, door={edge.IsDoor})", LogLevel.Info);
        }
    }

    private void OnAutonomyForceReplanCommand(string name, string[] args)
    {
        if (args.Length < 1)
        {
            Monitor.Log("Usage: slrpg_autonomy_force_replan <npc>", LogLevel.Info);
            return;
        }

        var npcName = args[0];
        if (!_autonomyRuntimeByNpcId.TryGetValue(npcName, out var runtime))
        {
            Monitor.Log($"No autonomy runtime found for '{npcName}'.", LogLevel.Warn);
            return;
        }

        var npc = ResolveNpcByName(npcName);
        var currentLoc = npc?.currentLocation?.Name ?? "Town";
        _npcAutonomyPlannerService?.ReplanWithFallback(_state, runtime, currentLoc, "forced_via_console");
        Monitor.Log($"Forced replan for {npcName} (replan #{runtime.ReplanCount}).", LogLevel.Info);
    }
        ...
        string? referencedNpcName = null)
    {
        ...
        {
            vanillaTownBlendRule = "CONTEXT_BLEND_RULE: Blend immediate continuity from VANILLA_DIALOGUE_CONTEXT with broader continuity from TOWN_MEMORY.";
        }
    ...
    private void BeginVanillaDialogueCaptureSession(NPC npc)
    {
        ...
        var npcDisplayName = string.IsNullOrWhiteSpace(npc.displayName)
            ? npc.Name
            : npc.displayName;
        if (!_recentVanillaDialogueByNpcToken.TryGetValue(npcToken, out var session) || session is null)
        {
            session = new RecentVanillaDialogueContext();
        }
        else if (session.Day != _state.Calendar.Day)
        {
        ...
        session.TimeOfDay = Game1.timeOfDay;
        session.CapturedUtc = DateTime.UtcNow;
    ...
    private void TryBackfillVanillaDialogueContextFromSpeaker(NPC npc)
    {
        if (npc is null || string.IsNullOrWhiteSpace(npc.Name))
            return;

        if (TryGetRecentVanillaDialogueContext(npc.Name, out var context)
            && (!string.IsNullOrWhiteSpace(context.LastDialogueLine) || context.DialogueSequence.Count > 0))
        {
            return;
        }
        ...
        AppendVanillaDialogueContextLine(npc, normalizedLine);
    }

    private void TryEnsureRecentVanillaDialogueContextForNpc(string? npcName)
    {
        ...
        if (Vector2.Distance(npc.Tile, Game1.player.Tile) > VanillaDialogueContextVicinityRadiusTiles)
            return;
